...
%***********************************************************************
\author{Duch\'e Axel$^{1,2}$, Gasso Gilles$^2$ and Chatelain Cl\'ément $^2$
%
...
Modern cities operate extensive camera networks across transportation hubs, public spaces, and sensitive zones. These systems must deliver reliable, continuous analytics---including motion detection, intrusion monitoring, and behavior analysis---under strict constraints on computation, storage, and energy consumption across thousands of concurrent video streams. Conventional image-domain preprocessing~\cite{PIDS_Survey_2022} (e.g., background subtraction or frame differencing) offers low-cost activity filtering but remains fragile under illumination changes, vibrations and clutter.

Deep learning--based vision models have substantially improved robustness and precision while maintaining a good balance between speed and accuracy, as seen in recent architectures such as RT-DETR~\cite{detrs_beat_yolos_2024} and the latest YOLO versions~\cite{khanam2024yolov11overviewkeyarchitectural}. However, most pipelines operate on fully decoded RGB frames, which are large and expensive to process. Performing real-time inference on high-resolution RGB data demands powerful GPUs or specialized hardware, making large-scale deployment difficult and energy-intensive. This dependency on heavy RGB processing represents a fundamental scalability barrier for camera networks.

We address this limitation by exploring whether high-level video analysis can be achieved without continuous RGB decoding. Our lightweight hybrid architecture performs a single detection on an initial decoded RGB frame, then propagates and updates bounding boxes directly from codec-domain features---motion vectors and discrete cosine transform (DCT) coefficients. This strategy markedly reduces the computational burden of deep inference while maintaining competitive accuracy relative to RGB-based pipelines, enabling scalable and energy-efficient analytics across large distributed camera infrastructures.

To situate our approach, we briefly review prior research on compressed-domain video understanding and how existing strategies balance efficiency and accuracy under similar constraints.


\section{Related Work and Positioning}
\label{sec:sota}

Research on efficient video understanding has explored several strategies to reduce the cost of processing full RGB frames.

\paragraph{Motion-aided tracking.}
Early works incorporated motion information produced by the video encoder to assist standard vision pipelines~\cite{mv_yolo_motion_vector_2018}. These methods combined the encoder’s built-in motion estimates with conventional RGB detections, improving temporal consistency but still relying on full image decoding.

\paragraph{Frame-group aggregation.}
Later approaches processed short frame sequences directly from the compressed stream to exploit temporal redundancy~\cite{wang2019fastobjectdetectioncompressed,Wang_2021}. They achieved good accuracy by modeling frame-to-frame changes, yet their design remained relatively heavy, making real-time operation on large networks difficult.

\paragraph{Partially decoded or hybrid models.}
More recent techniques~\cite{Fast_Object_Detection_in_High-Resolution_Videos_2023} mixed a small amount of decoded appearance with encoded motion cues. This hybrid design improved recognition quality but still required repeated RGB reconstruction, which limits scalability in large deployments. Moreover, decoding even small image regions is often inefficient, since video data are encoded sequentially within the stream—accessing a single patch or frame usually requires decompression of preceding segments, negating much of the intended speed gain.

\paragraph{Our positioning.}
Our earlier studies on learning directly from frequency-domain information~\cite{dct_detection_2019,dct_luminance_2021} showed that compact encoded data can retain the structure needed for object localization. Building on this idea, we propose a lightweight \emph{propagation model} that performs one detection on a decoded RGB frame and then updates bounding boxes using only motion and compact encoded features. This eliminates repeated RGB processing and achieves a practical balance between accuracy, efficiency, and scalability for real-time monitoring.


\section{Compressed Video Representation and Feature Extraction}
\label{sec:compressed_representation}

We operate directly on the \textbf{MPEG-4 Part 2} bitstream, common in surveillance systems, to exploit motion and frequency information already produced by the encoder.

Each sequence is encoded as \emph{Groups of Pictures} (GOPs) $\mathcal{G}^g=\{f_0^g,\dots,f_N^g\}$, where $f_0^g$ is an intra-coded (\textbf{I}) frame and later $f_n^g$ ($n>0$) are predictive (\textbf{P}) frames.  
The I-frame is a full image, while each P-frame stores:
\begin{itemize}
    \item \textbf{Motion vectors (MV)} — 2-D block displacements indicating apparent motion, similar to low-resolution optical flow;
    \item \textbf{Residuals} — local corrections encoded by an 8$\times$8 \textbf{Discrete Cosine Transform (DCT)} over luminance ($Y$).
\end{itemize}
Thus a predictive frame can be summarized as
\[
f_n^g=\{\mathcal{DCT}(\Delta Y_n^g),\,MV_n^g\},
\quad f_0^g=\{\mathcal{DCT}(Y_0^g)\},
\]
yielding a highly compact representation (Table~\ref{tab:data_ratio}).
